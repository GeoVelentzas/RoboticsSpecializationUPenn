...
map(~input_map) = 1;   % Mark free cel0000000000000000000000ls
...
while true
   ...
   % ********************************************************************* 
    ...
    N = Neighbors(current, input_map);
    
    for k = 1: size(N,2)
        n = N(:,k);
        if (map(n(1),n(2))==1) || (map(n(1),n(2))==6)
            distanceFromStart(n(1),n(2)) = min_dist+1;
...
%     
%     if j<ncols
%         if (map(i,j+1)==1) || (map(i,j+1) == 6) || (map(i,j+1) == 4)
...
%     
%     if i<nrows
%         if (map(i+1,j)==1) || (map(i+1,j) == 6) || (map(i+1,j) == 4)
